...
Torrent.prototype.extractTorrent = function (torrent_info, full) {
  ...
  if (torrent_info.swarm) {
    ...
    torrent.d_speed = prettyBytes(torrent_info.downloadSpeed)
    torrent.u_speed = prettyBytes(torrent_info.uploadSpeed)
...
Torrent.prototype.getTorrents = function (cb, currentUser) {
  ...
  if (typeof this.client !== 'undefined') {
    var get_torrents, get_torrent
    if (currentUser === 1) {
      get_torrents = this.torrent_db.find()
    } else {
      get_torrents = this.torrent_db.find({'user': currentUser})
    }
    for (var i in get_torrents) {
      get_torrent = this.client.get(get_torrents[i].infoHash)

      if (get_torrent && get_torrent.infoHash) {
...
Torrent.prototype.addTorrent = function (torrent, cb, currentUser) {
  ...
  this.client.add(torrent, {
    ...
    if (torrent.infoHash) {
      var check_torrent = self.torrent_db.find({'user': currentUser, infoHash: torrent.infoHash})
      if (!check_torrent.length) {
...
/**
 * Add all torrents again
 *
 *
 * @param {String} infoHash
 * @param {Function} cb
 */
Torrent.prototype.reloadTorrents = function () {
  var get_torrents = this.torrent_db.find()
  for (var i in get_torrents) {
    this.client.add(get_torrents[i].magnet)
  }
}
